...
import { JamedaFeedbackDialog } from './components';
 ...
 */
export function maybeOpenJamedaFeedbackDialog(conference: Object) {
    type R = {
    ...
    return (dispatch: Dispatch<any>, getState: Function): Promise<R> => {
        ...
        } else if (state['features/base/dialog'].component === JamedaFeedbackDialog) {
            // Feedback is currently being displayed.

            return Promise.reject(FEEDBACK_REQUEST_IN_PROGRESS);
        } else if (state['features/jameda/feedback'].submitted) {
 ...
 * @returns {Function}
 */
export function submitJamedaFeedback(
        score: number,
        message: string,
        conference: Object) {
